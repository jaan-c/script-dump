import numpy as np
from skimage import img_as_float64, data, io, color
from timeit import default_timer as timer

# TODO: This is the fastest implementation so far. Fancy indexing found in
# batched_dither.py seems to make the computation longer.
def dither_grayscale(image: np.ndarray, *, level_count: int = 4) -> np.ndarray:
    ...
    elif isinstance(image.dtype.type, np.float64):
        raise ValueError(f"image must contain numpy.float64..")
...
print(f"Unique pixels: {np.unique(dithered)}")
